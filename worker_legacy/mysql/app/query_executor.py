...
def detect_query_type(query: str) -> str:
    ...
    if query_trimmed.startswith("SELECT"):
        ...
        return "ANALYZE"
    elif query_trimmed.startswith("OPTIMIZE"):
        return "OPTIMIZE"
    elif query_trimmed.startswith("CHECK"):
        return "CHECK"
    elif query_trimmed.startswith("REPAIR"):
        return "REPAIR"
    elif query_trimmed.startswith("EXPLAIN"):
        return "EXPLAIN"
    elif query_trimmed.startswith("DESCRIBE") or query_trimmed.startswith("DESC"):
        return "DESCRIBE"
    elif query_trimmed.startswith("SHOW"):
        return "SHOW"
...
def execute_query(query: str, engine):
    ...
    try:
        with get_database_connection(engine) as conn:
            logger.info(f"[{datetime.datetime.now()}] Connected to MySQL database")
            result = conn.execute(text(query))

            if query_type == "SELECT" or query_type == "SHOW" or query_type == "DESCRIBE":
                # Handle SELECT-like queries - fetch rows
                ...
                for row in rows:
                    ...
                    for key, value in row._mapping.items():
                        # Handle binary data (BLOB, BINARY, VARBINARY) by converting to hex string
