...
import { connectSource, disconnectSource } from "@/shared/lib/api";
  ...
  return useMutation({
    ...
    onSuccess: async (_, sourceId) => {
      try {
        ...
        // Wait a moment and then try to load schemas
        await new Promise((resolve) => setTimeout(resolve, 1000));

        try {
          // Use the store's integrated schema loading
          await loadSourceSchema(sourceId);
          // If schema loading succeeds, set status to "running"
          setSourceStatus(sourceId, "running");
          queryClient.setQueryData(["sourceStatus", sourceId], "running");
        } catch (error) {
          console.error("❌ Error fetching source schemas:", error);
          // If schema loading fails, set status to "unhealthy"
          setSourceStatus(sourceId, "unhealthy");
          queryClient.setQueryData(["sourceStatus", sourceId], "unhealthy");
        }
      } finally {
