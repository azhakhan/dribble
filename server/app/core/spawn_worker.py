    ...
    def test(self):
        try:
            result = client.containers.run(
                ...
                detach=False,
                remove=True,
                command=["/bin/sh", "-c", "python -m app.main test-connection 2>&1"],
            )
...
class MySQLWorkerContainer:
    def __init__(self, source_id: UUID, creds: MysqlCreds):
        self.source_id = source_id
        self.creds = creds
        self.container_name = f"dribble-worker-mysql-{self.source_id}"
        self.port = 8000 + (int(random.randint(1, 999)))
        self.network_name = network_name
        self.container_url = f"http://{self.container_name}:8000"
        self.redis_url = os.environ.get("REDIS_URL", "redis://redis:6379")
    ...
    def start(self, db: Session = None):
        ...
        container = client.containers.run(
            image="dribble-worker-mysql:latest",
            name=self.container_name,
            environment={
                "DB_CREDS": self.creds.model_dump_json(),
                "REDIS_URL": self.redis_url,
        ...
        try:
            result = client.containers.run(
                ...
                command=[
                    "/bin/sh",
                    "-c",
                    "python -c 'from app.utils import test_connection_cli; test_connection_cli()' 2>&1",
                ],
            )
            output = result.decode("utf-8").strip()
            import json
            ...
            return {
                "status": "error",
                "message": "MySQL worker image not found. Please build the MySQL worker image first.",
            }
        except docker.errors.APIError as e:
            return {"status": "error", "message": f"Docker API error: {str(e)}"}
        except Exception as e:
...
def create_worker_container(source_id: UUID, dbtype: str, creds):
    """Factory function to create the appropriate worker container based on database type"""
    if dbtype == "postgres":
        return WorkerContainer(source_id, creds)
    elif dbtype == "mysql":
        return MySQLWorkerContainer(source_id, creds)
    else:
